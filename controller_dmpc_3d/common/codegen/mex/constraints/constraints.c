...
static emlrtRSInfo emlrtRSI = { 12,    /* lineNo */
  "constraints",                       /* fcnName */
  "/Users/root1/Desktop/Flocking_maneuvers/flock_maneuver/3D/controller_dmpc_3d/common/constraints.m"/* pathName */
};
                 ...
                 *params, real_T c_data[], int32_T c_size[1])
{
  ...
  if (z1_size_idx_1 == 0) {
    ...
    for (k = 0; k < z1_size_idx_1; k++) {
      nx = k * 3;
      res_data[k] = (z1_data[nx] + z1_data[nx + 1]) + z1_data[nx + 2];
    }
  }
  ...
  nx = z1_size_idx_1 - 1;
  for (k = 0; k <= nx; k++) {
    res_data[k] -= params->amax;
  }

  c_size[0] = z1_size_idx_1;
  if (0 <= z1_size_idx_1 - 1) {
    memcpy(&c_data[0], &res_data[0], z1_size_idx_1 * sizeof(real_T));
  }
