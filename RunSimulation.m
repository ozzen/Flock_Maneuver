...
%         if k >= floor(params.start_turn/control_jump) && k <= floor((params.start_turn+params.t_fix)/control_jump) %dot(P1,P2)/(norm(P1)*norm(P2))
        if k >= floor(params.start_turn/control_jump)
            vel = squeeze(s(2*k,4:6,:));
            avg_vel = mean(vel,2).';
            vel_init = squeeze(s(params.start_turn,4:6,:));
            avg_vel_init = mean(vel_init,2).';
            theta = atan2d(norm(cross(avg_vel_init(1,:),avg_vel(1,:))),dot(avg_vel_init(1,:),avg_vel(1,:)));
        else
            theta = 1000;
        end
        psi(k+1,1) = theta;
...
%                 else
%                     a(:,edge_agents(idx)) = trim_vec(w_turn*(a_past(:,edge_agents(idx)) + params.dt * 0.5 * params.jerk * acc_turn) + (1 - w_turn)*a(:,edge_agents(idx)), params.amax);
%                 end
